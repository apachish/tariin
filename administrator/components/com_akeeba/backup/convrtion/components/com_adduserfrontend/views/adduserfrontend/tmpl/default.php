...
while($parentgrouporgroup > 9) { // Loop to get parent_id untill we find a parent_id of the standard joomla usergroups
...
$sql = "SELECT COUNT(*) ".$db->quoteName('username')." FROM ".$db->quoteName('#__users')." WHERE ".$db->quoteName('username')." = ".$db->quote($username)."";
$db->setQuery($sql);
...
if ($emailexist == "1") {	
echo $sql = "SELECT COUNT(*) ".$db->quoteName('email')." FROM ".$db->quoteName('#__users')." WHERE ".$db->quoteName('email')." = ".$db->quote($email)." and id IN (select user_id from #__user_usergroup_map where group_id IN (".implode(',',permistionlevel())."))";
$db->setQuery($sql);
$num_rows = $db->loadResult();
if($num_rows == 0){
$email = $email;
...
".$db->quoteName('group_id')."        = ".$db->quote($usertype).",
...
foreach ($result as $line) {
    ...
    if(in_array($line[0],$per)){
        if(!($t%3))
            echo '<tr>';
    echo '<td><input type="checkbox" value='.$line[0].' name="group[]"><span>  '.$line[1].'</span></td>';
        if(!($t%3) || $t>=3)
            echo '</tr>';
            $t++;
    }
