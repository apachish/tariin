...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_membership&task=__membership_member4422s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', '__membership_member4422List', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<?php endif;?>
		...
		<table class="table table-striped" id="__membership_member4422List">
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, '__membership_member4422s.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_membership&task=__membership_member4422.edit&id='.(int) $item->id); ?>">
					<?php echo $this->escape($item->payment); ?></a>
				<?php else : ?>
					<?php echo $this->escape($item->payment); ?>
				<?php endif; ?>
				</td>
				<td>
